import { useNavigate } from "react-router-dom";

const ProtectRoute = () => {
  const navigate = useNavigate();

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-cyan-100 via-white to-cyan-100 px-4">
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-cyan-600 text-center mb-6">
        Please Login First
      </h1>
      <p className="text-center text-lg md:text-xl text-cyan-500 mb-8 max-w-lg">
        You need to be logged in to access this page. Click the button below to sign in.
      </p>
      <div className="flex gap-4">
        <button
          onClick={() => navigate("/")}
          className="px-8 py-3 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl shadow-lg transition-all duration-300 text-lg md:text-xl"
        >
          Home
        </button>
        <button
          onClick={() => navigate("/login")}
          className="px-8 py-3 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl shadow-lg transition-all duration-300 text-lg md:text-xl"
        >
          Sign In
        </button>
      </div>
    </div>
  );
};

export default ProtectRoute;
