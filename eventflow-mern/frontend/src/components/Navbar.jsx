...
export default function Navbar() {
  ...
  // Admin sees all management tools, normal users only see their booked tickets
  const privateLinks = user?.role === 'admin' 
    ? [
        { href: "/my-events", label: "My Events", icon: <MdEventNote /> },
        { href: "/ticket-booking", label: "My Bookings", icon: <GiTicket /> },
        { href: "/create-event", label: "Create Event", icon: <FaRegCalendarPlus /> },
        { href: "/admin", label: "Admin Panel", icon: <FaUser /> }
      ]
    : [
        { href: "/ticket-booking", label: "My Bookings", icon: <GiTicket /> },
      ];
    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md shadow-md border-b border-sky-400/30">
      <nav className="max-w-7xl mx-auto flex items-center justify-between h-16 px-5">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <div className="p-2 bg-gradient-to-r from-sky-600 to-cyan-500 rounded-lg text-white group-hover:scale-105 transition-transform">
        ...
        <div className="hidden lg:flex items-center gap-4">
          {status === "unauthenticated" ? (
            <div className="flex items-center gap-3">
              ...
              <Link
                to="/register"
                className="px-6 py-2 rounded-xl font-bold text-white bg-gradient-to-r from-sky-600 to-cyan-500 hover:shadow-lg hover:shadow-sky-200 transition-all active:scale-95 shadow-md"
              >
                Create Account
              </Link>
            </div>
              ...
              {status === "unauthenticated" ? (
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Link
                    to="/register"
                    onClick={() => setIsOpen(false)}
                    className="flex items-center justify-center h-12 rounded-xl font-bold text-white bg-gradient-to-r from-sky-600 to-cyan-500 shadow-lg shadow-sky-100"
                  >
                    Sign Up
                  </Link>
                </div>
