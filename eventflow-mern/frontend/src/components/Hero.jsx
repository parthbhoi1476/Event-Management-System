...
export default function Hero() {
  ...
  return (
    <section className="relative h-[85vh] w-full mt-1 overflow-hidden bg-gray-900 group">
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="max-w-4xl mx-auto px-6 text-center z-10">
              ...
              >
                <Link to={slides[currentSlide].link1 || "/all-events"}>
                ...
                {/* Only show secondary button if it's NOT pointing to create-event OR if the user is an admin */}
                {(slides[currentSlide].link2 !== "/create-event" || user?.role === 'admin') && (
                  <Link to={slides[currentSlide].link2 || "/create-event"}>
      ...
      <button
        onClick={prevSlide}
        className="absolute left-6 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-white hover:bg-white/20 transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
      >
      ...
      <button
        onClick={nextSlide}
        className="absolute right-6 top-1/2 -translate-y-1/2 z-20 p-3 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-white hover:bg-white/20 transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
      >
