...
export default function Profile() {
  ...
  const fetchUser = async () => {
    try {
      const res = await axiosClient.get(`/api/users/${user.email}`);
      const data = res.data;
      setFormData({
        name: data.name || "",
        image: data.image || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axiosClient.put(`/api/users/${user.email}`, formData);
    ...
    <div className="min-h-screen bg-[#f8fafc] py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <Link to="/" className="inline-flex items-center text-sm font-bold text-slate-500 hover:text-[#0069a8] transition-colors mb-2 group">
              ...
              <div className="relative group mb-6">
                ...
                >
                  <img
                    ...
                    alt="Profile"
                    className={`object-cover w-full h-full transition-opacity ${uploading ? 'opacity-30' : 'group-hover:opacity-100'}`}
                  />
