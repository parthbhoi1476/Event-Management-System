...
import { FaUser, FaEnvelope, FaLock, FaArrowRight, FaChevronLeft } from "react-icons/fa";
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const loginRes = await axiosClient.post("/api/auth/login", { email, password });
      ...
      Swal.fire({
        ...
        text: error.response?.data?.error || "Could not connect to the server",
        confirmButtonColor: "#0069a8",
        background: "#ffffff",
        customClass: {
          popup: "rounded-3xl shadow-2xl",
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc]">
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-[#0069a8]">
        <div className="absolute inset-0 z-0">
          <img
            ...
            className="object-cover w-full h-full opacity-60 mix-blend-overlay rotate-180"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#0069a8] via-transparent to-transparent opacity-80" />
        </div>
        
        <div className="relative z-10 w-full flex flex-col justify-between p-12 text-white">
          <Link to="/" className="flex items-center gap-2 text-2xl font-bold tracking-tighter hover:opacity-80 transition-opacity">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 lg:p-20 relative bg-white lg:rounded-l-[40px] shadow-[-20px_0_50px_rgba(0,0,0,0.05)] z-20">
        <div className="w-full max-w-md space-y-8">
          <Link to="/" className="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-[#0069a8] transition-colors mb-4 group">
          ...
          <div className="text-center">
            <p className="text-slate-600 font-medium">
              ...
              <Link to="/login" className="text-[#0069a8] font-extrabold hover:underline">
                Log In
              </Link>
            </p>
          </div>
