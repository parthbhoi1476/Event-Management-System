...
const MyEvents = () => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        const res = await axiosClient.get(`/api/events/my?email=${encodeURIComponent(userEmail)}`);
        setEvents(res.data);
      } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axiosClient.delete(`/api/events/${id}`);
                ...
                {events.map((event) => (
                  <tr key={event.id} className="hover:bg-gray-50 transition duration-150">
                    ...
                    <td className="px-4 py-3">
                      ...
                      >
                        {new Date(event.date) > currentDate ? "Upcoming" : "Completed"}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-700">
